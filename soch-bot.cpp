...
int main()
{
    ...
    enum class State {
        ...
        COLLECT_GENDER,
        COLLECT_DEPENDENTS,
        COLLECT_ANNUAL_INCOME,
        COLLECT_AVG_ELEC,
        COLLECT_CURR_ELEC,
        COLLECT_HAS_EXISTING,
        COLLECT_NUM_LOANS,
        COLLECT_LOAN_ACTIVE,
        COLLECT_LOAN_TOTAL,
        COLLECT_LOAN_RETURNED,
        COLLECT_LOAN_DUE,
        COLLECT_LOAN_BANK,
        COLLECT_LOAN_CATEGORY,
        COLLECT_REF1_NAME,
        COLLECT_REF1_CNIC,
        COLLECT_REF1_ISSUE,
        COLLECT_REF1_PHONE,
        ...
        RESUME_CNIC,
        GENERAL
    };  // Updated with personal, resume, and general states
    ...
    std::string resume_cnic;      // Added for resume
    ...
    while (true) {
        ...
        if (currentState == State::NORMAL) {
            ...
            if (id >= 1 &&
                ...
                std::string monthlyInfo =
                    personalSelection.calculateMonthlyPayment(
                        selected_personal
                    );
            ...
            if (low == "yes" || low == "y") {
                std::string resp =
                    utterHandler.getResponse("selected_installment");
                display.greetingResponse(resp);
                ...
                display.monthlyInstallmentDisplay(
                    total,
                    down,
                    insts,
                    rems
                ...
                std::string resp = utterHandler.getResponse("no_installment");
                display.greetingResponse(resp);
            ...
            if (low == "yes" || low == "y") {
                ...
                display.promptForInput(
                    "Do you have an existing application to resume? (yes/no)"
                );
            } else if (low == "no" || low == "n") {
                currentState = State::NORMAL;
            } else {
            ...
            m_str = userInput.substr(0, space);
            y_str = userInput.substr(space + 1);
            ...
            if (userInput == "back" || userInput == "exit") {
                ...
                display.greetingResponse(resp);
                   ...
                   currentState >= State::PROCEED_APPLY &&
                   currentState <= State::COLLECT_CONFIRM) {
            // Added pause handling in app states
            save_partial();
            currentState = State::NORMAL;
        }
    }
